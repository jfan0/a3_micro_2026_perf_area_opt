...
**The parent design you are improving — branch `baseline_area_v15_timing`:**

| metric | value |
|---|---|
| total cell area | 4,999,156 um2 |
| embench speed score | 105.605 |
| embench speed per MHz | 2.192332 |
| mean embench IPC | 1.428116 (reported, not the objective) |
| fmax | 48.17 MHz (tt_025C_1v80) |
| perf_area_eff (speed/mm2) | 21.124491 speed/mm2 |

**The bar you have to clear is not that last row.** It is `21.124491 speed/mm2`, reached
by branch `v15_timing` — the best figure any branch in the tree has measured,
and what the gate compares you against.

Usually `v15_timing` *is* the parent and the two are the same number. When
...
1. **`perf_area_eff` must exceed `21.124491 speed/mm2`** — the best any branch has
   measured (`v15_timing`), not the parent's figure. Strictly exceed:
...
`parent_eff x (your ratio) > 21.124491 speed/mm2`, so work out that gap in percent first
...
The same three files for each of: v15_1_synth_only_flatten, v15_1, v15_synth_only_flatten, seed_synth_only_flatten, v15 — the most recent
