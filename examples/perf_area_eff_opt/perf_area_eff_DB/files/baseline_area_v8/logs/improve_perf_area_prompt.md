...
**The parent design you are improving — branch `baseline_area_v7_1`:**

| metric | value |
|---|---|
| total cell area | 5,636,879 um2 |
| embench speed score | 99.751 |
| embench speed per MHz | 2.197645 |
| mean embench IPC | 1.432266 (reported, not the objective) |
| fmax | 45.39 MHz (tt_025C_1v80) |
| perf_area_eff (speed/mm2) | 17.696157 speed/mm2 |
...
| branch | built on | area mm2 | fmax MHz | IPC | speed/MHz | speed/mm2 | status | files this step touched |
|---|---|---|---|---|---|---|---|---|
| v7 | v6_2 | 5.612 | 38.77 | 1.378920 | 2.104870 | 14.540079 | gates_failed | -- |
| v8 | v7_1 | n/a | n/a | n/a | n/a | n/a | n/a | -- |
...
### v7 (gates_failed, on v6_2) -- area -5.9%  cycles -5.2%  fmax -16.2%

```diff
index 1c35493a..8518b632 100644
@@ -292,11 +292,20 @@ class BoomDuplicatedDataArray(implicit p: Parameters) extends AbstractBoomDataAr
         size = nSets * refillCycles,
...
@@ -468,7 +477,12 @@ class BoomNonBlockingDCacheModule(outer: BoomNonBlockingDCache) extends LazyModu
     data.io.read(w).valid := dataReadArb.io.out.bits.valid(w) && dataReadArb.io.out.valid
     data.io.read(w).bits  := dataReadArb.io.out.bits.req(w)
   }
-  dataReadArb.io.out.ready := true.B
...
+  dataReadArb.io.out.ready := !dataWriteArb.io.out.valid
   ...
   data.io.write.valid := dataWriteArb.io.out.fire
   data.io.write.bits  := dataWriteArb.io.out.bits
@@ -576,10 +590,33 @@ class BoomNonBlockingDCacheModule(outer: BoomNonBlockingDCache) extends LazyModu
```
