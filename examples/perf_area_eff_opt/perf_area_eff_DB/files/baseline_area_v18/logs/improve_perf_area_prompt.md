...
**The parent design you are improving — branch `baseline_area_v17`:**

| metric | value |
|---|---|
| total cell area | 4,993,670 um2 |
| embench speed score | 116.831 |
| embench speed per MHz | 2.192370 |
| mean embench IPC | 1.428141 (reported, not the objective) |
| fmax | 53.29 MHz (tt_025C_1v80) |
| perf_area_eff (speed/mm2) | 23.395899 speed/mm2 |

**The bar you have to clear is not that last row.** It is `23.395899 speed/mm2`, reached
by branch `v17` — the best figure any branch in the tree has measured,
and what the gate compares you against.

Usually `v17` *is* the parent and the two are the same number. When
...
| v11 | v10 | 5.476 | 48.17 | 1.432266 | 2.197645 | 19.332549 | ok | -- |
| v12 | v11 | 5.482 | 38.65 | 1.432266 | 2.197645 | 15.494041 | gates_failed | fpu.scala |
| v13 | v12 | 5.306 | 48.17 | 1.432266 | 2.197645 | 19.952759 | ok | -- |
| v14 | v13 | 5.136 | 48.17 | 1.428717 | 2.193479 | 20.571389 | ok | -- |
| v15 | v14 | 4.999 | 43.48 | 1.428116 | 2.192332 | 19.068051 | gates_failed | -- |
| v15_timing | v15 | 4.999 | 48.17 | 1.428116 | 2.192332 | 21.124491 | ok | issue-unit.scala |
| v16 | v15_timing | 5.025 | 52.37 | 1.428141 | 2.192370 | 22.850188 | ok | fpu.scala |
| v17 | v16 | 4.994 | 53.29 | 1.428141 | 2.192370 | 23.395899 | ok | functional-unit.scala, pipelined-mul.scala |
...
| branch | built on | area mm2 | fmax MHz | IPC | speed/MHz | speed/mm2 | status | files this step touched |
|---|---|---|---|---|---|---|---|---|
| v18 | v17 | n/a | n/a | n/a | n/a | n/a | n/a | -- |
...
1. **`perf_area_eff` must exceed `23.395899 speed/mm2`** — the best any branch has
   measured (`v17`), not the parent's figure. Strictly exceed:
...
`parent_eff x (your ratio) > 23.395899 speed/mm2`, so work out that gap in percent first
...
## Earlier branches' reports — `/tmp/improve_perf_area/history/<branch>/`

The same three files for each of: v16, v15_timing, v15_1_synth_only_flatten, v15_1, v15_synth_only_flatten — the most recent
...
- **What did that edit actually break?**
  `grep -E '^Endpoint' /tmp/improve_perf_area/history/<branch>/sta_report.rpt | sort | uniq -c | sort -rn | head`
- **Did it hurt the structure you think it hurt?**
  `grep -c '<module_name>' /tmp/improve_perf_area/history/<branch>/sta_report.rpt` — zero
...
- **Where its area went:**
  `grep 'Chip area for module' /tmp/improve_perf_area/history/<branch>/area_report.txt | sort -t: -k2 -g -r | head -20`
- **Which benchmarks moved:** `cat /tmp/improve_perf_area/history/<branch>/ipc_report.md`
...
### v12 (gates_failed, on v11) -- area +0.1%  cycles +0.0%  fmax -19.8%
...
+
+  val s1_mulAddA   = Pipe(io.validin, mulAddRecFNToRaw_preMul.io.mulAddA,   1)
+  val s1_mulAddB   = Pipe(io.validin, mulAddRecFNToRaw_preMul.io.mulAddB,   1)
+  val s1_mulAddC   = Pipe(io.validin, mulAddRecFNToRaw_preMul.io.mulAddC,   1)
+  val s1_toPostMul = Pipe(io.validin, mulAddRecFNToRaw_preMul.io.toPostMul, 1)
+  val s1_rm        = Pipe(io.validin, io.roundingMode,                      1)
+  val s1_tininess  = Pipe(io.validin, io.detectTininess,                    1)
+  val s1_valid     = s1_mulAddA.valid
+
+  // ---- stage 1: the sigWidth x sigWidth multiply-accumulate --------------
+  val mulAddResult = (s1_mulAddA.bits * s1_mulAddB.bits) +& s1_mulAddC.bits
+
+  val s2_mulAddResult = Pipe(s1_valid, mulAddResult,      1)
+  val s2_toPostMul    = Pipe(s1_valid, s1_toPostMul.bits, 1)
+  val s2_rm           = Pipe(s1_valid, s1_rm.bits,        1)
+  val s2_tininess     = Pipe(s1_valid, s1_tininess.bits,  1)
+  val s2_valid        = s2_mulAddResult.valid
+
+  // ---- stage 2: normalize + round ----------------------------------------
+  val mulAddRecFNToRaw_postMul = Module(new hardfloat.MulAddRecFNToRaw_postMul(expWidth, sigWidth))
+  mulAddRecFNToRaw_postMul.io.fromPreMul   := s2_toPostMul.bits
+  mulAddRecFNToRaw_postMul.io.mulAddResult := s2_mulAddResult.bits
+  mulAddRecFNToRaw_postMul.io.roundingMode := s2_rm.bits
+
...
+  roundRawFNToRecFN.io.roundingMode   := s2_rm.bits
+  roundRawFNToRecFN.io.detectTininess := s2_tininess.bits
+
+  val s3_out = Pipe(s2_valid, roundRawFNToRecFN.io.out,            1)
+  val s3_exc = Pipe(s2_valid, roundRawFNToRecFN.io.exceptionFlags, 1)
+
+  io.out            := s3_out.bits
+  io.exceptionFlags := s3_exc.bits
+  io.validout       := s3_out.valid
+}
+
+/**
+ * FMA pipe built on BoomMulAddRecFNPipe3.
+ *
+ * Identical to rocket's FPUFMAPipe -- same input register, same operand
+ * fixups, same total latency -- except that the FMA underneath has three
+ * register boundaries instead of two and therefore needs no trailing output
+ * register at latency 4.  Nothing upstream of the input register changes, so
+ * the cone that feeds it (FP register read -> unbox/fuInput) is exactly the
+ * length it was.
+ */
+class BoomFPUFMAPipe(val latency: Int, val t: tile.FType)(implicit p: Parameters)
...
+  }
+
+  val fma = Module(new BoomMulAddRecFNPipe3(t.exp, t.sig))
...
+
+  io.out := Pipe(fma.io.validout, res, latency-4)
+}
```

### v13 (ok, on v12) -- area -3.2%  cycles +0.0%  fmax +24.6%

```diff
index d8e2183f..ee818113 100644
...
### v14 (ok, on v13) -- area -3.2%  cycles -0.2%  fmax +0.0%

```diff
index d8e2183f..75f462e4 100644
@@ -543,26 +545,25 @@ class WithNCS152DefaultBooms(n: Int = 1) extends Config(
...
### v15 (gates_failed, on v14) -- area -2.7%  cycles -0.1%  fmax -9.7%
...
### v15_timing (ok, on v15) -- area +0.0%  cycles +0.0%  fmax +10.8%

```diff
index 40cb5ae6..3deda87d 100644
@@ -172,12 +172,30 @@ class IssueSlot(val numWakeupPorts: Int)(implicit p: Parameters)
...
@@ -292,4 +310,51 @@ class IssueSlot(val numWakeupPorts: Int)(implicit p: Parameters)
diff --git a/src/main/scala/v3/exu/issue-units/issue-unit.scala b/src/main/scala/v3/exu/issue-units/issue-unit.scala
index 8afa10f2..c07c5c97 100644
--- a/src/main/scala/v3/exu/issue-units/issue-unit.scala
+++ b/src/main/scala/v3/exu/issue-units/issue-unit.scala
@@ -218,11 +218,12 @@ abstract class IssueUnit(
...
+  // Squashed grants: grants that will be squashed because ld_miss arrived and operand was poisoned.
+  // Same value as PopCount(grant && ld_miss && poisoned), but ld_miss is ANDed in
+  // once on the narrow sum instead of once per slot -- as written before, this
+  // diagnostic counter alone was a quarter of the load on the late-arriving
+  // ld_miss net.  perf_issued_poison above is already (grant && poisoned).
+  io.perf_dep.squash_grants := Mux(io.ld_miss, PopCount(perf_issued_poison), 0.U)
 
   //-------------------------------------------------------------
```

### v16 (ok, on v15_timing) -- area +0.5%  cycles +0.0%  fmax +8.7%

```diff
index d8e2183f..f81cba66 100644
-              fpu = Some(freechips.rocketchip.tile.FPUParams(sfmaLatency=4, dfmaLatency=4, divSqrt=true))
...
diff --git a/src/main/scala/v3/exu/execution-units/fpu.scala b/src/main/scala/v3/exu/execution-units/fpu.scala
index 074170d2..ec0e698d 100644
--- a/src/main/scala/v3/exu/execution-units/fpu.scala
+++ b/src/main/scala/v3/exu/execution-units/fpu.scala
@@ -205,7 +205,7 @@ class FPU(implicit p: Parameters) extends BoomModule with tile.HasFPUParameters
     req
...
@@ -249,3 +249,149 @@ class FPU(implicit p: Parameters) extends BoomModule with tile.HasFPUParameters
   io.resp.bits.fflags.valid      := io.resp.valid
   io.resp.bits.fflags.bits.flags := fpu_out_exc
 }
+
...
+  mulAddRecFNToRaw_preMul.io.c  := io.c
+
+  // s0 -> s1: operand alignment only.  This is the boundary rocket shares with
+  // the multiply and the reason the tile misses timing.
+  val s1_mulAddA   = Pipe(io.validin, mulAddRecFNToRaw_preMul.io.mulAddA,   1)
+  val s1_mulAddB   = Pipe(io.validin, mulAddRecFNToRaw_preMul.io.mulAddB,   1)
+  val s1_mulAddC   = Pipe(io.validin, mulAddRecFNToRaw_preMul.io.mulAddC,   1)
+  val s1_toPostMul = Pipe(io.validin, mulAddRecFNToRaw_preMul.io.toPostMul, 1)
...
+  val s1_valid     = s1_mulAddA.valid
+
+  // s1 -> s2: the fused multiply-add, one expression so abc still merges the
+  // multiplier's final carry-propagate with the addend's.
+  val mulAddResult = (s1_mulAddA.bits * s1_mulAddB.bits) +& s1_mulAddC.bits
+
+  val s2_mulAddResult = Pipe(s1_valid, mulAddResult,      1)
+  val s2_toPostMul    = Pipe(s1_valid, s1_toPostMul.bits, 1)
...
+  val s2_valid        = s2_mulAddResult.valid
+
+  // s2 -> s3: postMul, exactly rocket's stage.
+  mulAddRecFNToRaw_postMul.io.fromPreMul   := s2_toPostMul.bits
+  mulAddRecFNToRaw_postMul.io.mulAddResult := s2_mulAddResult.bits
+  mulAddRecFNToRaw_postMul.io.roundingMode := s2_rm.bits
+
+  // s3: round, exactly rocket's stage.
+  val roundRawFNToRecFN = Module(new hardfloat.RoundRawFNToRecFN(expWidth, sigWidth, 0))
+  roundRawFNToRecFN.io.invalidExc     := Pipe(s2_valid, mulAddRecFNToRaw_postMul.io.invalidExc, 1).bits
+  roundRawFNToRecFN.io.in             := Pipe(s2_valid, mulAddRecFNToRaw_postMul.io.rawOut,     1).bits
+  roundRawFNToRecFN.io.roundingMode   := Pipe(s2_valid, s2_rm.bits,                             1).bits
+  roundRawFNToRecFN.io.detectTininess := Pipe(s2_valid, s2_tininess.bits,                       1).bits
+  io.validout                         := Pipe(s2_valid, false.B,                                1).valid
+
+  roundRawFNToRecFN.io.infiniteExc := false.B
+
...
+
+/**
+ * FMA pipe built on BoomMulAddRecFNPipe.
+ *
+ * Line-for-line rocket's FPUFMAPipe -- same input register, same operand
+ * fixups, same trailing Pipe -- except the inner pipe holds one more register
+ * stage, so the trailing Pipe is (latency-4) rather than (latency-3).  At
+ * dfmaLatency = 5 that is one output register, the same as rocket has at 4, and
+ * the total register count from io.in to io.out is 5.
+ *
+ * Everything feeding the input register and everything downstream of the output
+ * register therefore sees paths of exactly the length they had.  Those are the
+ * two boundaries earlier attempts loaded up: putting preMul ahead of the input
+ * register (-20.2% fmax) and merging postMul with round (-19.8% fmax).
+ */
+class BoomFPUFMAPipe(val latency: Int, val t: tile.FType)(implicit p: Parameters)
+  extends BoomModule with HasFPUParameters
+{
...
+  res.exc := fma.io.exceptionFlags
+
+  io.out := Pipe(fma.io.validout, res, (latency-4) max 0)
+}
```

### v17 (ok, on v16) -- area -0.6%  cycles +0.0%  fmax +1.8%
...
diff --git a/src/main/scala/v3/exu/execution-units/pipelined-mul.scala b/src/main/scala/v3/exu/execution-units/pipelined-mul.scala
new file mode 100644
index 00000000..864959d1
--- /dev/null
+++ b/src/main/scala/v3/exu/execution-units/pipelined-mul.scala
@@ -0,0 +1,111 @@
+//******************************************************************************
...
+ *
+ * The algebra, with a1/a0 and b1/b0 the high/low halves of the two operands and
+ * sa/sb the (masked) sign bits:
+ *
+ *   lhs = in1 - sa*2^64 , rhs = in2 - sb*2^64
+ *   lhs*rhs = in1*in2 - (sa*in2 + sb*in1)*2^64 + sa*sb*2^128
+ *
+ * Only 128 bits of the product are ever read, so the sa*sb*2^128 term vanishes
+ * and the correction is a plain 64-bit value applied at bit 64:
+ *
+ *   prod[127:0] = a0*b0 + (a1*b0 + a0*b1)<<32 + (a1*b1 - corr)<<64   (mod 2^128)
+ *
+ * corr is built straight from the s0 operands, so it is off the multiplier's
+ * critical path and is folded into a1*b1's register.  prod[31:0] is a0*b0[31:0]
+ * verbatim, so stage 2 only has to add the top 96 bits.
+ */
...
+  val hw = width / 2
+
+  // ---- s0: the input register rocket has, unchanged -----------------------
+  val in = Pipe(io.req)
+
+  // Exactly the three control bits rocket's DecodeLogic minimises to over
+  // {FN_MUL=0, FN_MULH=1, FN_MULHSU=2, FN_MULHU=3}:
+  //   cmdHi = fn[0]|fn[1], lhsSigned = !(fn[1]&fn[0]), rhsSigned = !fn[1]
+  val cmdHi     =   in.bits.fn(0) || in.bits.fn(1)
+  val lhsSigned = !(in.bits.fn(1) && in.bits.fn(0))
+  val rhsSigned = !(in.bits.fn(1))
+  val cmdHalf   = (width > 32).B && in.bits.dw === 0.U  // DW_32 === false.B
+
+  val a1 = in.bits.in1(width-1, hw)
+  val a0 = in.bits.in1(hw-1, 0)
+  val b1 = in.bits.in2(width-1, hw)
+  val b0 = in.bits.in2(hw-1, 0)
+  val sa = lhsSigned && in.bits.in1(width-1)
+  val sb = rhsSigned && in.bits.in2(width-1)
+
+  // corr = sa*in2 + sb*in1, taken mod 2^64 (it is applied at bit 64 and the
+  // product is only ever read 128 bits wide).  Depends on s0 operands only.
+  val corr = Mux(sa, in.bits.in2, 0.U(width.W)) + Mux(sb, in.bits.in1, 0.U(width.W))
+
+  // ---- s0 -> s1: four unsigned hw x hw products ---------------------------
+  val s1_valid   = RegNext(in.valid, false.B)
+  val s1_p00     = RegEnable(a0 * b0, in.valid)                 // 2*hw bits
+  val s1_p10     = RegEnable(a1 * b0, in.valid)
+  val s1_p01     = RegEnable(a0 * b1, in.valid)
+  val s1_p11     = RegEnable(a1 * b1 - corr, in.valid)          // sign correction folded in
+  val s1_cmdHi   = RegEnable(cmdHi, in.valid)
+  val s1_cmdHalf = RegEnable(cmdHalf, in.valid)
+
+  // ---- s1: recombine.  prod[hw-1:0] needs no adder at all. ----------------
+  val hi = (Cat(0.U((2*hw).W), s1_p00(width-1, hw))   // p00 >> 32,   at bit 32
+          + Cat(0.U(hw.W),     s1_p10)                // a1*b0,       at bit 32
+          + Cat(0.U(hw.W),     s1_p01)                // a0*b1,       at bit 32
+          + Cat(s1_p11,        0.U(hw.W)))            // a1*b1-corr,  at bit 64
+  val prod = Cat(hi, s1_p00(hw-1, 0))                 // 2*width bits
+
+  val muxed = Mux(s1_cmdHi,   prod(2*width-1, width),
+              Mux(s1_cmdHalf, Cat(Fill(hw, prod(hw-1)), prod(hw-1, 0)),
+                              prod(width-1, 0)))
+
+  // ---- s1 -> out: the remaining register(s) of the fixed latency ----------
+  val out = Pipe(s1_valid, muxed, latency-2)
+  io.resp.valid          := out.valid
+  io.resp.bits.data      := out.bits
+  io.resp.bits.tag       := DontCare
+  io.resp.bits.full_data := DontCare
+}
```
