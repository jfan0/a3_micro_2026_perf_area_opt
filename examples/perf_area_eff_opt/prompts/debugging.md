...
- Use Ray or any other cluster mechanism (`import ray`, `ray.init`, the `ray`
  CLI, Ray/gRPC proxy environment variables), or run anything on another node.
  `chipyard_bash` on the build node is your whole interface.
